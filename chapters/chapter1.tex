...
\section{Reminder of some basic concepts}
\label{haussdorf}
...
\begin{definition}[Haussdorf distance]
    ...
    $$
\end{definition}
Notice this is not an actuall metric (distance), since $d(A,B)=0$ 
does not imply $A=B$. (Obviously it is a metric if we quotient module
rigid motions).

We will also need to compare functions:

\begin{definition}
    \label{fnorm}
    Let $M$ be a manifold.
    We define $\norm{f}_\infty$ to be the usual $L_\infty$ form:
    $$
        \norm{f}_\infty
        =\sup\{
            \vert f(x) \vert : x\in M 
        \}
    $$
    And $\norm{f}_\infty^i$ is defined as:
    $$
        \norm{f}_\infty^i
        =
        \max\{
